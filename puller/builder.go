...
func accountHashKey(a *models.Account) uint64 {
...
type Builder struct {
	...
	accHashMap map[uint64]*models.Account           // hash(acc_hash) -> *Account (both known and new accounts)
	accMap     map[models.AccountID]*models.Account // id -> *Account (both known and new accounts)
	dlgHashMap map[uint64]*models.Account           // delegates by hash
	dlgMap     map[models.AccountID]*models.Account // delegates by id

	// build state
	block     *models.Block
	parent    *models.Block
	baking    []models.Right
	endorsing []models.Right
	branches  map[string]*models.Block
}

func NewBuilder(idx *Indexer) *Builder {
	return &Builder{
		idx:        idx,
		accHashMap: make(map[uint64]*models.Account),
		accMap:     make(map[models.AccountID]*models.Account),
		dlgMap:     make(map[models.AccountID]*models.Account),
		dlgHashMap: make(map[uint64]*models.Account),
		baking:     make([]models.Right, 0, 64),
		endorsing:  make([]models.Right, 0, 32),
		branches:   make(map[string]*models.Block, 128), // more than max of 64
	}
}

func (b *Builder) RegisterDelegate(acc *models.Account) {
	acc.IsDelegate = true
...
func (b *Builder) UnregisterDelegate(acc *models.Account) {
	acc.DelegateId = 0
...
func (b *Builder) ActivateDelegate(acc *models.Account) {
	acc.IsActiveDelegate = true
	acc.IsDirty = true
}

func (b *Builder) DeactivateDelegate(acc *models.Account) {
	acc.IsActiveDelegate = false
	acc.IsDirty = true
}

// AccountByAddress 判断此地址是否已经存在缓存中
func (b *Builder) AccountByAddress(addr chain.Address) (*models.Account, bool) {
...
func (b *Builder) AccountById(id models.AccountID) (*models.Account, bool) {
	// lookup delegate accounts first
...
func (b *Builder) BranchByHash(h chain.BlockHash) (*models.Block, bool) {
	branch, ok := b.branches[h.String()]
	return branch, ok
}

func (b *Builder) Accounts() map[models.AccountID]*models.Account {
	return b.accMap
}

func (b *Builder) Delegates() map[models.AccountID]*models.Account {
	return b.dlgMap
}

func (b *Builder) Rights(typ chain.RightType) []models.Right {
	switch typ {
...
func (b *Builder) Init(ctx context.Context, tip *models.ChainTip, c *rpc.Client) error {
	if tip.BestHeight < 0 {
		return nil
	}
...
func (b *Builder) Clean() {
	...
	// clear build state
	b.accHashMap = make(map[uint64]*models.Account)
	b.accMap = make(map[models.AccountID]*models.Account)
	b.baking = b.baking[:0]
...
func (b *Builder) Purge() {
	...
	b.dlgHashMap = make(map[uint64]*models.Account)
	b.dlgMap = make(map[models.AccountID]*models.Account)

	// clear branches (keep most recent 64 blocks only)
	for _, v := range b.branches {
		v.Free()
	}
	b.branches = make(map[string]*models.Block, 128)
...
func (b *Builder) BuildReorg(ctx context.Context, tz *models.Bundle, parent *models.Block) (*models.Block, error) {
	// use parent as builder parent
	b.parent = parent

	// build reorg block
	var err error
	if b.block, err = models.NewBlock(tz, parent); err != nil {
...
func (b *Builder) UpdateStats(ctx context.Context) error {
	...
	erights := make(map[models.AccountID]int)
	var count int
	for _, r := range b.endorsing {
...
func (b *Builder) BuildGenesisBlock(ctx context.Context) (*models.Block, error) {
	...
	for _, v := range gen.Accounts {
		...
		acc := models.NewAccount(v.Addr)
		acc.RowId = models.AccountID(len(accounts) + 1)
		acc.FirstSeen = b.block.Height
		acc.LastSeen = b.block.Height
		...
		f := models.NewFlow(b.block, acc, acc)
		f.Category = models.FlowCategoryBalance
		f.Operation = models.FlowTypeActivation
	...
	for _, v := range gen.Contracts {
		...
		if v.Delegate.IsValid() {
			...
			f := models.NewFlow(b.block, dlg, acc)
			f.Category = models.FlowCategoryDelegation
			f.Operation = models.FlowTypeDelegation
...
func (b *Builder) RunBabylonAirdrop(ctx context.Context, params *chain.Params) error {
	...
	var accs []*models.Account
	err := b.idx.statedb.Where("address_type = ?", int64(chain.AddressTypeContract)).Find(&accs).Error
	if err != nil {
		return err
	}
	for _, acc := range accs {
		...
		if acc.IsContract {
			if !acc.IsSpendable && !acc.IsDelegatable {
				return nil
			}
		}
		managers = append(managers, acc.ManagerId.Value())
		return nil
	}

	// find unfunded managers
	var count int
	var accss []*models.Account
	err = b.idx.statedb.Where("is_funded = ? and row_id in (?)", false, util.UniqueUint64Slice(managers)).Find(&accss).Error
	if err != nil {
		return err
	}
	for _, acc := range accss {
		...
		flow := models.NewFlow(b.block, acc, nil)
		flow.Category = models.FlowCategoryBalance
		flow.Operation = models.FlowTypeAirdrop
		flow.AmountIn = 1
...
func (b *Builder) PatchBigMapDiff(ctx context.Context, diff micheline.BigMapDiff, accId models.AccountID, script *micheline.Script) (micheline.BigMapDiff, error) {
